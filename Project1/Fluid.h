	...
	for (int i = 0; i < NUMPART; i++)
	
			{
				...
				*/
				...
				p[i].nextPosition = boundToZero(glm::vec3(xIndex,yIndex,zIndex),Fluid::source);
...
void Fluid::Draw(GLint modelLoc, glm::mat4 model, GLint objectColorLoc, Shader lightingShader)
{
	// Set up vertex data (and buffer(s)) and attribute pointers
	GLuint VBO, particleVAO;
	FIREDRAW::initBuffer(VBO, particleVAO);
	...
	for (int i = 0; i < NUMPART; i++)
			{
					...
					FIREDRAW::DrawParticles(particleVAO, objectColorLoc, p[i].color,texture,transparency, lightingShader);

			}

}

void Fluid::setSize(particle &p)
{
	p.particleSize = p.particleSize/(1.01+0.0005*p.distance);
}

void Fluid::setColor(particle &p)
{
...
{
	position = Fluid::source();
	u0 = glm::vec3(rand()%10000, 0, rand()%10000);
	u1 = glm::vec3(0, 0, 0);
	particleSize = 7;
	distance = 0;
...
void Fluid::loadParticleTexture()
{
...
void Fluid::decreaseExtinction()
{
	extinction = extinction>0.01?extinction-0.01:0;
}

void Fluid::increaseExtinction()
{
	extinction += 0.01;
}